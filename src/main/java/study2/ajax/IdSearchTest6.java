...
public class IdSearchTest6 extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// JSP에서만, vo를 vos에 담을 수 없음... (SPRING에서는 가능!)
		...
		memberArray.add(memberInfo);		// 앞에서 json형식으로 저장한 map객체를 json배열객체에 add시켜준다. (vo에서 꺼내서 vos 담듯이 map에 담은 것)
		
		// 1건의 자료(vo)를 더 넣어보자. 1건의 자료를 더 담아주기위한 처리.(map에 담은후 JSONObject에 담아준다 : 총 2건의 자료가 담겼다.) -> while문 같은 것
		map = new HashMap<>();
		...
		memberArray.add(memberInfo);				// JSONOBject에 담긴자료를 JSONArray객체에 추가시켜준다.
