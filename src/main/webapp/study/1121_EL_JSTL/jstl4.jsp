...
%>
	<p>
		...
		<form name="myform">
			...
			<c:set var="cnt" value="0" />
			<c:set var="input1" value="0" />
			<c:set var="input2" value="0" />
			...
			<c:choose>
				<c:when test="${input2 != 0}">입력하신 문자의 두번째 위치 : ${input2} </c:when>
				<c:when test="${input1 != 0}">입력하신 문자의 첫번째 위치 : ${input1}(두번째 값 없음) </c:when>
