...
%>
	...
	<h4>num1배열에 담긴 값을 forEach문을 통해 출력</h4>
	<c:forEach var="num" items="${num1}" varStatus="st">
		${st.count}.(num1[${st.index}]) ${num}<br/>
		<!-- 상황에 맞게, st.count(1부터카운트) or st.index(인덱스번호) 사용 -->
	</c:forEach>
	<br/>
...
%>
	...
	== 비교 : ${var1 == "" }<br/>
	== 비교 : ${var2 == "" }<br/>
	== 비교 : ${var3 == "" }<br/> <!-- 들어있는 '값'을 비교하므로 false -->
	null 비교 : ${var1 == null }<br/>
	null 비교 : ${var2 == null }<br/> 
	null 비교 : ${var3 == null }<br/><br/> <!-- true -->

	연산자 : empty, !empty (null & 공백 동일 취급 -> 매우 편함)<br/>
	"" 와 empty 비교 : ${empty var1}<br/>
	"" 와 empty 비교 : ${empty var2}<br/>
	"" 와 empty 비교 : ${empty var3}<br/>
	null 과 !empty 비교 : ${!empty var1}<br/>
	null 과 !empty 비교 : ${!empty var2}<br/>
	null 과 !empty 비교 : ${!empty var3}<br/> 
