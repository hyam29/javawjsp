...
<div class="container">
	...
	<table class="table table-borderless">
		<tr>
			<td class="text=left p-0">
				<c:if test="${sLevel != 1}"><a href="${ctp}/boInput.bo" class="btn btn-outline-secondary btn-sm">글쓰기</a></c:if>
			</td>
	...
	<table class="table table-hover text-center">
		...
		<%-- <c:set var="curScrStartNo" value="${curScrStartNo}" /> 넣는 게 정석인데 생략해도 됨 --%>
		<c:forEach var="vo" items="${vos}">
			<tr>
				...
				<td>${vo.hour_diff > 24 ? fn:substring(vo.wDate,0,10) : fn:substring(vo.wDate,11,19)}</td> <!-- 시간로 비교 -->
				<td>${vo.readNum}</td>
				<td>${vo.good}</td>
			</tr>
